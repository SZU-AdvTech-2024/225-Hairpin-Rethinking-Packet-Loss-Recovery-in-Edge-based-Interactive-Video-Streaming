...
bool GameServer::IsRtxTimeout (Ptr<DataPacket> pkt, Time rto) {
  ...
  if (m_fecPolicy->GetFecName () == "HairpinPolicy" && pkt->GetTXCount () > 1) {
    ...
    //uint16_t remainingTime = static_cast<uint16_t>((m_delayDdl - (now - encodeTime)).GetMilliSeconds());
    //m_fecPolicy->SetFecCnt(statistic->curLossRate, m_frameDataPktCnt[m_nextFrameId] , remainingTime, rtt, statistic);
...
void GameServer::CheckRetransmission () {
  ...
  for (auto it = m_dataPktHistoryKey.end () - 1; isLoop && !m_dataPktHistoryKey.empty (); ) {
    ...
    if (shouldRtx) {
      ...
      //在这里更新重传冗余率
      uint16_t rtt = (uint16_t) ceil ((statistic->srtt + 1 * statistic->rttSd).GetMilliSeconds ());
      uint16_t ddlLeft= floor((m_delayDdl - (now - pkt->GetEncodeTime ())).GetMicroSeconds()/1e3);
      uint16_t ddl=m_delayDdl.GetMilliSeconds();
      //uint16_t remainingTime= std::max (0, int (ddlLeft) - int (rtt));
      uint16_t frameSize=pkt_queue.size();
      uint8_t packet=50;
      
      m_fecPolicy->UpdateBetaArrayToFile(statistic,m_sender->GetNetworkStatistics ()->curLossRate, frameSize, ddlLeft, rtt, packet, ddl);
      */
    }
