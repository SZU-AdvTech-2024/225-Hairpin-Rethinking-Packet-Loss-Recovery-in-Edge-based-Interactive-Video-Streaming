...
uint8_t HairpinPolicy::GetFecCnt (double_t loss, uint8_t frameSize, uint16_t remainingTime, uint16_t rtt, uint8_t packet) {
    ...
    /* assignment */
    return m_betaArray[index]+1;
}

void HairpinPolicy::UpdateBetaArrayToFile(Ptr<NetStat> statistic, double_t loss, uint8_t frameSize, uint16_t remainingTime, uint16_t rtt, uint8_t packet, uint16_t ddl) {
    packet = GetBlockSize (statistic->curLossRate, frameSize, ddl, rtt, statistic->oneWayDispersion.GetMicroSeconds () / 1e3);
    ...
    char buf[100];
    snprintf(buf, sizeof(buf), "beta-rtx%d-cap%d-coeff%.0e.bin", m_isRtx, m_isCap, k_qoeCoeff);
    std::string filePath = k_paramDir + buf;

    std::fstream file(filePath, std::ios::in | std::ios::out | std::ios::binary);
    if (!file.is_open()) {
        NS_FATAL_ERROR("Cannot open file: " + filePath);
        return;
    }

    uint64_t offset = index * sizeof(uint8_t);

    file.seekg(0, std::ios::end);
    uint64_t fileSize = file.tellg();
    if (offset >= fileSize) {
        NS_FATAL_ERROR("Index out of bounds: " + std::to_string(index));
        file.close();
        return;
    }

    file.seekp(offset, std::ios::beg);
    uint8_t value = m_betaArray[index];
    file.write(reinterpret_cast<const char*>(&value), sizeof(uint8_t));
    if (!file) {
        NS_FATAL_ERROR("Failed to write data to file: " + filePath);
    }

    file.close();
    NS_LOG_INFO("Updated beta array index " + std::to_string(index) + " to file: " + filePath);
}
